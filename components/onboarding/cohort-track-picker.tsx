  ...
  return (
    <section className="flex min-h-0 flex-1 flex-col justify-start py-3 sm:pt-8 sm:pb-12">
      <div className="mx-auto w-full max-w-4xl px-3 sm:px-0">
        <h1 className="mb-4 text-center font-heading text-2xl font-extrabold text-nga-primary sm:mb-6 sm:text-3xl">
          Pick your track
        </h1>
        <div className="grid grid-cols-1 divide-y divide-[#031F82]/10 sm:grid-cols-3 sm:items-start sm:gap-8 sm:divide-y-0">
          {MASTERY_COHORT_ORDER.map((cohort) => {
            ...
            return (
              ...
              >
                <span className="relative size-28 shrink-0 sm:aspect-square sm:h-auto sm:w-full sm:max-w-[16rem] sm:size-auto">
                  <Image
                    src={TRACK_AVATAR_SRC[cohort]}
                    alt={`${label} avatar`}
                    fill
                    sizes="(max-width: 639px) 112px, 256px"
                    className="object-contain"
                    unoptimized
                  />
                </span>
                <span className="flex min-w-0 flex-1 flex-col justify-center sm:flex-none sm:items-center">
                  <span className="font-heading text-2xl font-black leading-tight text-nga-primary sm:mt-3">
                    {label}
                  </span>
                  <span className="mt-0.5 font-heading text-sm font-medium leading-tight text-nga-ink/65 sm:mt-1">
                    {ageRange} years
                  </span>
                </span>
              </button>
